...
import colinhewlettsolutions.client.controller.exceptions.*;
...
import colinhewlettsolutions.client.model.entity.PatientPrimaryCondition;
import colinhewlettsolutions.client.model.entity.PatientCondition;
...
import colinhewlettsolutions.client.model.non_entity.SecondaryConditionWithState;
import colinhewlettsolutions.client.model.non_entity.QuestionWithState;
import colinhewlettsolutions.client.model.entity.PatientSecondaryCondition;
import colinhewlettsolutions.client.model.non_entity.PrimaryConditionWithState;
import colinhewlettsolutions.client.model.non_entity.ConditionWithState;
import colinhewlettsolutions.client.model.non_entity.TableRow;
...
import static colinhewlettsolutions.client.model.entity.Question.Category.ARE_YOU;
import static colinhewlettsolutions.client.model.entity.Question.Category.HAVE_YOU;
import static colinhewlettsolutions.client.model.entity.Question.Category.YOU_AND_THE_CLINIC;
import static colinhewlettsolutions.client.controller.SystemDefinition.FONT.DYNAMIC;
import static colinhewlettsolutions.client.controller.SystemDefinition.FONT.TICK;
import static colinhewlettsolutions.client.controller.SystemDefinition.ScheduleTableWithContactDetails.PHONE;
...
public abstract class ViewController implements ActionListener, PropertyChangeListener{
    ...
    public static enum DesktopViewControllerActionEvent{
        USER_SYSTEM_WIDE_FACTORY_SETTINGS_REQUEST,//handled here because could arise from several different view controllers
        USER_SYSTEM_WIDE_SETTINGS_REQUEST,//ditto
    ...
    public static enum DesktopViewControllerPropertyChangeEvent{
        USER_SYSTEM_WIDE_SETTINGS_RECEIVED, //handled here because request raised in different view controllers
    ...
    public static enum ScheduleViewControllerActionEvent{
        ...
        USER_SCHEDULE_DIARY_FACTORY_SETTINGS_REQUEST,//request default settings (for no user or 'guest')
        USER_SCHEDULE_DIARY_SETTINGS_EDITOR_VIEW_REQUEST,//request schedule diary settings view (editor) for current user
        USER_SCHEDULE_DIARY_SETTINGS_REQUEST,//request for current schedule diary settings for this user
        USER_SCHEDULE_DIARY_SETTINGS_UPDATE_REQUEST,
        USER_SCHEDULE_LIST_FACTORY_SETTINGS_REQUEST,//request default settings (for no user or 'guest')
        USER_SCHEDULE_LIST_SETTINGS_EDITOR_VIEW_REQUEST,//request schedule list settings view (editor) for current user
        USER_SCHEDULE_LIST_SETTINGS_REQUEST,//request for current schedule list settings for this user
        USER_SCHEDULE_LIST_SETTINGS_UPDATE_REQUEST,
        
        
        /** note, updates and inserts of user settings not handles by the ScheduleViewController; handles by the UserSettingsViewController*/
    ...
    public static enum UserSettingsViewControllerActionEvent{
        USER_SCHEDULE_DIARY_FACTORY_SETTINGS_REQUEST,
        USER_SCHEDULE_DIARY_SETTINGS_UPDATE_REQUEST,
    ...
    public static enum UserSettingsViewControllerPropertyChangeEvent{
        DIARY_BOOKABLE_SLOT_BACKGROUND_CHANGE_RECEIVED,
        DIARY_BOOKABLE_SLOT_FOREGROUND_CHANGE_RECEIVED,
        DIARY_BOOKING_FIRST_SLOT_BACKGROUND_CHANGE_RECEIVED,
        DIARY_BOOKING_FIRST_SLOT_FOREGROUND_CHANGE_RECEIVED,
        DIARY_BOOKING_REMAINING_SLOTS_BACKGROUND_CHANGE_RECEIVED,
        DIARY_BOOKING_REMAINING_SLOTS_FOREGROUND_CHANGE_RECEIVED,
        DIARY_EMERGENCY_BOOKING_BACKGROUND_CHANGE_RECEIVED,
        DIARY_EMERGENCY_BOOKING_FOREGROUND_CHANGE_RECEIVED,
        DIARY_UNBOOKABLE_SLOT_BACKGROUND_CHANGE_RECEIVED,
        DIARY_UNBOOKABLE_SLOT_FOREGROUND_CHANGE_RECEIVED,
    ...
    public final ViewController getMyController(){
        return viewController;
    }
    protected final void setMyController(ViewController value){
    ...
    protected Appointment doChangeAppointmentScheduleForDayRequest(ViewMode mode, Appointment rSlot) throws StoreException{
        ...
        if (getScheduleReport().getState().equals(RequestedAppointmentState.COLLISION)){
            getDescriptor().getControllerDescription().setProperty(SystemDefinition.Properties.ERROR,scheduleReport.getError());
            result = null;
        }
        else{
    ...
    */
    ...
    protected void doPrintPatientMedicalHistoryQuestionnaireRequest(boolean isNotForNewPatient)throws StoreException{
        setPatient((Patient)getDescriptor().getControllerDescription().getProperty(SystemDefinition.Properties.PATIENT));
    ...
    private ConditionWithState getConditionWithStateFromCondition(Condition condition)throws StoreException{
        ...
        if (condition.getIsPrimaryCondition()){
            ...
            pac = new PatientPrimaryCondition((Patient)
                    getDescriptor().getControllerDescription().getProperty(SystemDefinition.Properties.PATIENT),pc );
            pac.setScope(Entity.Scope.SINGLE);
            pac = pac.read();
            if (pac!=null){
            ...
            pac = new PatientSecondaryCondition(
                    (Patient)getDescriptor().getControllerDescription().getProperty(SystemDefinition.Properties.PATIENT),sc );
            pac.setScope(Entity.Scope.SINGLE);
            pac = pac.read();
            if (pac!=null){
...
*/
    ...
    private void populateAppointmentScheduleTable(XWPFTable table){
        ...
        ArrayList<Appointment> appointments = (ArrayList<Appointment>)getDescriptor().getControllerDescription().getProperty(SystemDefinition.Properties.APPOINTMENT_SLOTS_FOR_DAY_IN_LIST_FORMAT);
    ...
    private void populatePatientMedicalHistoryTable(XWPFTable table)throws StoreException{
        ...
        pc = (PrimaryCondition)getDescriptor()
                .getControllerDescription().getProperty(SystemDefinition.Properties.PRIMARY_CONDITION);
        for (Condition c : pc.get()){
        ...
        try{
            ...
            if (!appointment.get().isEmpty()){
                if (appointment.get().get(0).getStart().
                    ...
                    getDescriptor().getControllerDescription().
                            setProperty(SystemDefinition.Properties.APPOINTMENT_EARLY_START, appointment.get().get(0).getStart());
                }else getDescriptor().getControllerDescription().
                            setProperty(SystemDefinition.Properties.APPOINTMENT_EARLY_START,null);
            }
            if (!appointment.get().isEmpty()){
                if (appointment.get().get(appointment.get().size()-1).getStart().
                        isAfter(day.atTime(ViewController.LAST_APPOINTMENT_SLOT))){
                    getDescriptor().getControllerDescription().setProperty(SystemDefinition.Properties.APPOINTMENT_LATE_START, 
                            appointment.get().get(appointment.get().size()-1).getStart());
                }else getDescriptor().getControllerDescription().
                            setProperty(SystemDefinition.Properties.APPOINTMENT_LATE_START,null);
            }
            /**
            ...
            getDescriptor().getControllerDescription().setProperty(
                    SystemDefinition.Properties.APPOINTMENT_SLOTS_FOR_DAY_IN_LIST_FORMAT, appointmentSlotsForDay);
        }
        catch (StoreException ex){
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
            String message = ex.getMessage() 
