...
import static controller.ViewController.NotificationViewControllerPropertyChangeEvent.RECEIVED_PATIENT_NOTIFICATION;
...
public class ModalToDoEditorView extends ModalView implements ActionListener{
    ...
    private void doReceivedToDo(){
        ToDo toDo = getMyController().getDescriptor().getControllerDescription().getToDo();
        if (toDo==null) {
    ...
    private boolean doValidateToDoRequest(){
        ...
        if (result){
            ...
            if (getViewMode().equals(ViewController.ViewMode.Create))
                ...
                toDo = getMyController().
                        getDescriptor().getControllerDescription().getToDo();
