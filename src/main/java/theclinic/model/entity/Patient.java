...
//</editor-fold>
 ...
 //<editor-fold defaultstate="collapsed" desc="Persistent store-related operations">
    ...
    public long getLastAppointmentThisManyMonthsAgo()throws StoreException{
        LocalDate when = getAppointmentHistory().get(0).getStart().toLocalDate();
        YearMonth now = YearMonth.now();
        return ChronoUnit.MONTHS.between(when, now);
    }
    ...
    public Integer insert() throws StoreException{
        ...
        if (getIsKeyDefined()){//option followed if data migration is happening
            pid = getRepository().insert(this,getKey(), null);
        }
        else {
            if (getIsGuardianAPatient()){//this option taken if a new patient record is being created
                pid = getRepository().insert(this, getKey(),getGuardian().getKey());
                if (pid==null){
                   ...
                   pid = getRepository().insert(this,getKey(), pid);
                }
            }
            else pid = getRepository().insert(this,getKey(), null);
        }
        setKey(pid); 
    ...
    public void update() throws StoreException{ 
        ...
        if (getIsGuardianAPatient()) 
            if (getGuardian()!=null)
                getRepository().update(this, this.getKey(),
                        this.getGuardian().getKey());
            else getRepository().update(this, this.getKey(),null);
        else {
            getRepository().update(this, this.getKey(),null);
        }
 ...
 //<editor-fold defaultstate="collapsed" desc="Public interface not including persistent storage related operatioms">
    ...
    private LocalDate lastNonGBTRecallSentDate = null;
    public LocalDate getLastNonGBTRecallSentDate() {
        return lastNonGBTRecallSentDate;
    }
    public void setLastNonGBTRecallSentDate(LocalDate nonGbtRecallSentDate) {
        this.lastNonGBTRecallSentDate = nonGbtRecallSentDate;
    }

    private LocalDate lastGBTRecallSentDate = null;
    public LocalDate getLastGBTRecallSentDate() {
        return lastGBTRecallSentDate;
    }
    public void setLastGBTRecallSentDate(LocalDate lastGBTRecallSentDate) {
        this.lastGBTRecallSentDate = lastGBTRecallSentDate;
    }
    
    private Boolean isRequestToSendPatientGBTRecallPending = false;
    public Boolean getIsRequestToSendPatientGBTRecallPending() {
        return isRequestToSendPatientGBTRecallPending;
    }
    public void setIsRequestToSendPatientGBTRecallPending(Boolean requestToSendPatientRecall) {
        this.isRequestToSendPatientGBTRecallPending = requestToSendPatientRecall;
    }
    
    private Boolean isRequestToSendPatientNonGBTRecallPending = false;
    public Boolean getIsRequestToSendPatientNonGBTRecallPending() {
        return isRequestToSendPatientNonGBTRecallPending;
    }
    public void setIsRequestToSendPatientNonGBTRecallPending(Boolean requestToSendPatientRecall) {
        this.isRequestToSendPatientNonGBTRecallPending = requestToSendPatientRecall;
    }
    ...
    public class Name {
        ...
        public String getWithForenamesInitialised(){
            String[] parts = getForenames().trim().split("\\s+");
 ...
 //<editor-fold defaultstate="collapsed" desc="Data migration operations">
    ...
    public List<String[]> importEntityFromCSV()throws StoreException{
        return getRepository().importEntityFromCSV(this);
    }
