...
public class PatientAppointmentDataTableModel extends DefaultTableModel{
    ...
    public void setValueAt(Object value, int row, int col) {
        ...
        if (col==0){
            ...
            if (pad.getPatient().getIsRequestToSendPatientNonGBTRecallPending()){
                pad.getPatient().setIsRequestToSendPatientNonGBTRecallPending(Boolean.FALSE);
            }
            else{
                pad.getPatient().setIsRequestToSendPatientNonGBTRecallPending(Boolean.TRUE);
            }
            fireTableCellUpdated(row, col);
        }else if (col==1){
            PatientAppointmentData pad = data.get(row);
            ...
            if (pad.getPatient().getIsRequestToSendPatientGBTRecallPending()){
                pad.getPatient().setIsRequestToSendPatientGBTRecallPending(Boolean.FALSE);
            }
            else{
                pad.getPatient().setIsRequestToSendPatientGBTRecallPending(Boolean.TRUE);
            }
            fireTableCellUpdated(row, col);
        }  */ 
    }
