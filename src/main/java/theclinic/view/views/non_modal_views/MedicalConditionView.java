...
import theclinic.controller.MedicalConditionViewController;
...
import theclinic.view.support_classes.table_models.MedicalConditionTableModel;
...
public class MedicalConditionView extends View 
    ...
    private void setPrimaryCondition(PrimaryCondition pc){
        getMyController().getDescriptor()
                .getViewDescription().setProperty(SystemDefinition.Properties.PRIMARY_CONDITION, pc);
    }
    ...
    private Treatment getTreatment(){
        ...
        result = (Treatment)getMyController().getDescriptor()
                .getControllerDescription().getProperty(SystemDefinition.Properties.TREATMENT);
        return result;
    }
    
    private PrimaryCondition getPrimaryCondition(){
        PrimaryCondition result = null;
        result = (PrimaryCondition)getMyController().getDescriptor()
    ...
    private void populateConditionTable(){
        ...
        switch(getConditionViewMode()){
            case PRIMARY:
                conditions = ((PrimaryCondition)getMyController().getDescriptor()
                        .getControllerDescription()
                        .getProperty(SystemDefinition.Properties.PRIMARY_CONDITION)).get();
                break;
            case SECONDARY:
                conditions = ((PrimaryCondition)getMyController().getDescriptor()
                        .getControllerDescription()
                        .getProperty(SystemDefinition.Properties.PRIMARY_CONDITION)).getSecondaryCondition().get();
                this.btnDeleteMedicalCondition.setEnabled(false);
                this.btnRenameMedicalCondition.setEnabled(false);
                this.btnRenameMedicalCondition.setEnabled(false);
                break;
